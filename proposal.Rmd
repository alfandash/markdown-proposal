...
We can say `valence` variable that can help us to predict one track can be more cheerfull (positive) or can be sad (negative). let see the distribution of valance in our dataset.
...
From dataset we have, we try to visualize valence grouping with genre. This visualize can help us know, which genre that can give more positive or negative.   

```{r}
genre_valence <- tracks %>% select(valence, genre) %>% group_by(genre) %>% summarise("average_valence" = mean(valence))

ggplot(data=genre_valence, mapping = aes(x = reorder(genre,average_valence), y = average_valence, fill = genre)) +
  geom_col() +
  coord_flip() +
  theme_minimal() +
  theme(
...
Based on visualize above, we can see Reegae is music genre with highest average valence. It mean genre is the most genre can bring more positiveness and in our case it can help people release their stress.   

From dataset we get, we found we have `genre` whish it we can group our data base on it. We will visualize data, Later it can help us for choose genre is really popular, so if people dont like any genre we suggest we can give them by popularity
...
I will visualize correalation plot that shows correlation for every numerical variable. `Valence` is variable that we will pay attention more than others, because this variable inteprate how music can be cheerful or can be sad. Based on visualize, valence have correlation with most of others variable, onlu speechiness, liveness, and popularity have low correlation.
